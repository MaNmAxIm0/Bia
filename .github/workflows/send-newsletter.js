...
async function createAndSendCampaignForLanguage(config) {
  if (!config.groupId || !config.subject || !config.message) {
    console.log(
      `A saltar a língua '${config.lang}' por falta de configuração.`,
    );
    return;
  }
  console.log(
  ...
  try {
    ...
    console.log(
      `SUCESSO! Campanha (${config.lang}) criada com ID: ${campaignId}`,
    );
    ...
    console.log(
      `Comando de envio para a campanha (${config.lang}) emitido com sucesso!`,
    );
  } catch (error) {
...
async function sendAllNewsletters() {
  if (!API_KEY) {
    console.error(
      "Erro: A variável de ambiente MAILERLITE_API_KEY está em falta.",
    );
    process.exit(1);
  }
