...
def process_video(input_path: Path, output_path: Path, apply_watermark_flag: bool) -> bool:
  ...
  filter_complex = r"scale=min(1920\,iw):-2"
  if apply_watermark_flag:
    font_path = str(config.WATERMARK_FONT_PATH).replace('\\', '/')
    watermark_text = config.WATERMARK_TEXT.replace("'", "'")
    watermark_filter = (
      fr",drawtext=fontfile='{font_path}':text='{watermark_text}':"
      fr"fontsize=min(w\,h)*{config.VID_WATERMARK_FONT_RATIO}:fontcolor=black@0.5:"
      fr"x=(w-text_w-(min(w\,h)*{config.MARGIN_RATIO}))+2:y=(h-text_h-(min(w\,h)*{config.MARGIN_RATIO}))+2,"
      fr"drawtext=fontfile='{font_path}':text='{watermark_text}':"
      fr"fontsize=min(w\,h)*{config.VID_WATERMARK_FONT_RATIO}:fontcolor=white@0.8:"
      fr"x=w-text_w-(min(w\,h)*{config.MARGIN_RATIO}):y=h-text_h-(min(w\,h)*{config.MARGIN_RATIO})"
    )
