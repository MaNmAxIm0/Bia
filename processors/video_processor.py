...
import config
from .image_processor import process_image  # Reutiliza o processador de imagem
...
def apply_video_watermark(video_path: Path, output_path: Path) -> bool:
    ...
    try:
        ...
        logging.error(f"FFmpeg falhou ao processar vídeo '{video_path.name}': {e.stderr}")
        return False

def generate_thumbnail(video_path: Path, output_dir: Path) -> bool:
    ...
    try:
        ...
        if not process_image(temp_frame_path, final_thumb_path):
            raise Exception("Falha ao aplicar marca de água no thumbnail do vídeo.")
